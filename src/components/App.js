...
class App extends Component {
  ...
  render() {
    ...
    if (!user) {
      headerLogin = (
        <div className="app-header--login" key="login-comp">
          <button className="button button--login" onClick={this._handleLogin}>
            Join / Log In <span>via</span> <i className="icon-github" />
          </button>
        </div>
      );
    } else {
    ...
    if (user) {
      headerForm = <Form user={user} key="form-comp" inBZNJS={inBZNJS} />;
    }

    intro = (
      <ReactFitText compressor={2} minFontSize={40} maxFontSize={64}>
  ...
  _handleLogin = () => {
    ...
    Firebase.auth()
      ...
      .then(function(result) {
        ...
        Axios.get("https://api.github.com/user/orgs?access_token=" + token).then(function(orgs) {
